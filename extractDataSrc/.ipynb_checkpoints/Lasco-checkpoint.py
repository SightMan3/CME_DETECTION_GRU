...
class Lasco:
    ...
    def create_screenshot(self, df: pd.DataFrame, target_dir: str, hvpy_layer: DataSource, detector: str, scale: float=None, skip_images=0, verbose=True):
        ...
        for index in iterable:
            ...
            meta_data = self._get_metadata(image_id)
            if meta_data:
    ...
    def _get_metadata(self, image_id: str):
        """Fetch metadata for given image ID from Helioviewer"""
        try:
            url = "https://api.helioviewer.org/v2/getImageMetadata/"
            resp = requests.get(url, params={"id": image_id})
            if resp.status_code == 200:
                return resp.json()
        except Exception as e:
            print(f"Metadata fetch failed for ID {image_id}: {e}")
        return None
